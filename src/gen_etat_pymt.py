...
def create_etat_paiement_xlsx(df, output_file, titre_activite=None, date_activite=None, lieu_deroulement=None, date_paiement=None):
    ...
    headers_merged_in_cols = [
         ('E', 'FORFAIT PERDIEM \nALLER RETOUR SIMPLE'),  # E:F
        ('G', 'FORFAIT PERDIEM 1er \nET DERNIER JOUR DE \nMISSION (11250+25000+11250)'),  # G:H
        ('I', 'FORFAIT PERDIEM \nJOUR COMPLET'),  # I:J
    ]

    
    # Write headers for the current iteration
    ...
    for col, header in headers_merged_in_cols:
        ...
        # put border on the left, top and bottom of current cell and on the right of the next cell
        cell.border = Border(
            left=Side(style='thin'),
            top=Side(style='thin'),
            bottom=Side(style='thin')
        )
        next_col = chr(ord(col) + 1)  # Get the next column letter
    ...
    # define rows to repeat on each page
    ws.print_title_rows = '1:15'  # Repeat from row 1 to 15 on each page

    # Write data
    ## Calculate number of iterations needed
    nb_iteration = (len(df_final) + 7) // 8  # 8 rows per iteration
    start_block_row = 16  # or wherever you start
    block_height = 12

    for i in range(nb_iteration):
        data_start_row = start_block_row + (i * block_height)
    
        df_iteration = df_final.iloc[i * 8:(i + 1) * 8]
        # Pad with empty rows if needed
        rows_needed = 8 - len(df_iteration)
        if rows_needed > 0:
            empty_rows = pd.DataFrame([{}] * rows_needed, columns=df_final.columns)
            df_iteration = pd.concat([df_iteration, empty_rows], ignore_index=True)
        
        for row_num_in_batch, (_, row) in enumerate(df_iteration.iterrows()):
            current_row = data_start_row + row_num_in_batch

            # Write data for each column
            ws[f'A{current_row}'] = row_num_in_batch + 1  # Row number
            ...
            # Wrap text for all current row cells
        ...
        # df_iteration['MONTANT TOTAL'] est vide ou nulle
        Montant_total = 0
        ...
        # Apply gray fill to specified cells in total row
        gray_cells = ['A', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M', 'O', 'P','Q']
        for col in gray_cells:
            ws[f'{col}{data_start_row + 8}'].fill = gray_fill
            ws[f'{col}{data_start_row + 8}'].border = border_thin_all
        ...
        # Convert Montant_Total into words
    ...
    # Mege cells for column [E:F, G:H, I:J] for row 14 and 14 + (nb_iteration * 12)
    for i in range(nb_iteration):
        data_start_row = 14
        ws.merge_cells(start_row=data_start_row, start_column=5, end_row=data_start_row, end_column=6)  # E:F
        ws.merge_cells(start_row=data_start_row, start_column=7, end_row=data_start_row, end_column=8)  # G:H
        ws.merge_cells(start_row=data_start_row, start_column=9, end_row=data_start_row, end_column=10)  # I:J

    # Set orientation to landscape
    ws.page_setup.orientation = ws.ORIENTATION_LANDSCAPE

    # Add page breaks after each iteration
    for i in range(1, nb_iteration):
        break_row = 14 + (i * 13)
        ws.row_breaks.append(Break(id=break_row, max=1, min=1))
    
    # Set page margins
    ws.page_margins = PageMargins(left=0.2, right=0.2, top=0.2, bottom=0.05, header=0.3, footer=0.01)
    
    # Set print options
    last_row = summary_row  # from last iteration
    ws.print_area = f'A1:Q{last_row}'

    # Set all columns to one page
    ws.page_setup.fitToPage = True  # Enable fit to page mode
    ws.page_setup.fitToWidth = 1
    ws.page_setup.fitToHeight = 0  # 0 means unlimited height
